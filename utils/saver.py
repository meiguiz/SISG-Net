...
class Saver:
    def __init__(self, path, logdir, modeldir, imgdir):
        self.path = path            # 保存路径
    ...
    def save_summary(self):
        ...
        :return:
        """
        save_folder = os.path.join(self.path, self.logdir)
    ...
    def save_model(self, net, model_name):
        ...
        :return:
        """
        model_path = os.path.join(self.path, self.modeldir)
    ...
    def save_img(self, epoch, idx, imgs):
        ...
        save_folder = os.path.join(self.path, self.imgdir)
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
