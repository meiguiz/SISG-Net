...
def compute_loss(net, xc, target, xs, targett,  device):
    ...
    able_pred, angle_pred, width_pred, loss_dict = net(xc, xs, targett)         # shape 同上
    ...
    return {
        'loss': 10*(able_loss + angle_loss * 20 + width_loss),
        'losses': {
            'able_loss': able_loss,
            'angle_loss': angle_loss * 20,
...
def get_pred(net, xc, xs):
    able_pred, angle_pred, width_pred, outputs = net(xc, xs)
    """
    ...
    angle_pred = torch.sigmoid(angle_pred)
    width_pred = torch.sigmoid(width_pred)
    return able_pred, angle_pred, width_pred, outputs
