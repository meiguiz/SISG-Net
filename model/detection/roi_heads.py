...
def paste_mask_in_image(mask, box, im_h, im_w):
    ...
    im_mask = torch.zeros((384, 512), dtype=mask.dtype, device=mask.device)
    x_0 = max(box[0], 0)
    x_1 = min(box[2] + 1, 512)
    y_0 = max(box[1], 0)
    y_1 = min(box[3] + 1, 384)
...
class RoIHeads(nn.Module):
    ...
    def forward(self,
        ...
        if self.training:
            ...
            labels_1 = None
            boxes_1, scores_1, labels_1 = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
            num_images = len(boxes_1)
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                pos_matched_idxs = []
                for img_id in range(num_images):
                    pos = torch.where(labels_1[img_id] > 0)[0]
            ...
            if self.mask_roi_pool is not None:
                ...
                # feature_map_num = mask_logits.shape[0]
                # if self.training:
                #     feature_1 = torch.zeros(2,32,28,28).to(device)
                # else:
                #     feature_1 = torch.zeros(1,32,28,28).to(device)
                # for index in range(feature_map_num):  # 通过遍历的方式，将64个通道的tensor拿出
                #
                #     feature = mask_logits[index]
                ...
                # feature_1 = feature_1 / feature_map_num
                #print(feature_1.max())
            else:
